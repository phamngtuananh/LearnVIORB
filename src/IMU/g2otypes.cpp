...
void EdgePRIDP::linearizeOplus()
{
    ...
    Matrix3d J_pi_rcb = -SO3::hat(Rcic0 * (P0 - tcb)) * Rcb + Rcic0 * SO3::hat(P0 - tcb) * Rcb - Rcb * SO3::hat(RiT * (t0 - ti));
...
void EdgeNavStatePRV::computeError()
{
    ...
    const Vector3d rVij = RiT * (Vj - Vi - GravityVec * dTij) - (dVij + M.getJVBiasg() * dBgi + M.getJVBiasa() * dBai); //this line includes correction term of bias change
...
void EdgeNavStatePVR::computeError()
{
    ...
    // residual error of Delta Position measurement
    Vector3d rPij = RiT * (Pj - Pi - Vi * dTij - 0.5 * GravityVec * dT2) - (dPij + M.getJPBiasg() * dBgi + M.getJPBiasa() * dBai); // this line includes correction term of bias change.
    // residual error of Delta Velocity measurement
    Vector3d rVij = RiT * (Vj - Vi - GravityVec * dTij) - (dVij + M.getJVBiasg() * dBgi + M.getJVBiasa() * dBai); //this line includes correction term of bias change
    // residual error of Delta Rotation measurement
    ...
    if ((NSPVRi.Get_BiasGyr() - NSBiasi.Get_BiasGyr()).norm() > 1e-6 || (NSPVRi.Get_BiasAcc() - NSBiasi.Get_BiasAcc()).norm() > 1e-6)
    {
        ...
        std::cerr << "bias gyr not equal for PVR/Bias vertex" << std::endl
                  << NSPVRi.Get_BiasGyr().transpose() << " / " << NSBiasi.Get_BiasGyr().transpose() << std::endl;
        std::cerr << "bias acc not equal for PVR/Bias vertex" << std::endl
                  << NSPVRi.Get_BiasAcc().transpose() << " / " << NSBiasi.Get_BiasAcc().transpose() << std::endl;
    }
}
...
void EdgeNavStatePriorPVRBias::computeError()
{
    ...
    if ((nsPVRest.Get_BiasGyr() - nsBiasest.Get_BiasGyr()).norm() > 1e-6 || (nsPVRest.Get_BiasAcc() - nsBiasest.Get_BiasAcc()).norm() > 1e-6)
    {
        std::cerr << "bias gyr not equal for PVR/Bias vertex in EdgeNavStatePriorPVRBias" << std::endl
                  << nsPVRest.Get_BiasGyr().transpose() << " / " << nsBiasest.Get_BiasGyr().transpose() << std::endl;
        std::cerr << "bias acc not equal for PVR/Bias vertex in EdgeNavStatePriorPVRBias" << std::endl
                  << nsPVRest.Get_BiasAcc().transpose() << " / " << nsBiasest.Get_BiasAcc().transpose() << std::endl;
    }
}
